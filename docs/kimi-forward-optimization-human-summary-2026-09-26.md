...
这次优化把完整专家 E896、top-k=16、8 卡、每卡 4k tokens 的 fused forward 从本机复现 PR #68 的 17.655 ms 降到历史最佳稳定结果 15.095 ms；同次 Torch 基线为 21.697 ms，加速比 1.437x。清理后使用当前源码复验得到 15.129 ms、Torch 21.740 ms、1.437x，未见明显性能回退。相对 PR #68，fused 延迟下降 2.560 ms，约 14.5%。仍没有达到用户最初要求的 1.6x。
...
E896 每 rank 有 112 个本地专家，历史均匀路由诊断中约 7168 个 source/expert bucket 全部非空，每个 bucket 平均只有约 73 行；E32 的 bucket 更少、更厚。E896 因此更容易暴露 dispatch lane 空转、尾块填充、专家权重工作集、缓存和跨卡同步成本。
