...
把这两个数的差值当作连续流水带来的加速。
在相同的轮转调度下进一步比较更小的 FC1 tile：M128/N512/K128 为
7.742 ms，M64/N512/K128 为 12.422 ms，允许后者使用 L0C 多缓冲后为
12.714 ms；M64/N256/K128 和 M128/N256/K128 分别为 15.025/9.160 ms，
M64/N512/K256 为 19.485 ms。六组均通过三个抽样专家的完整输出检查。
固定 M64 的实际尾块填充率为 5.46%，低于 M128 的 9.62%，但增加的 tile
与重复工作抵消了这部分收益，未接入。把原有双累加改为显式传递给 `tl.dot`
的累加器后仍是 7.741 ms，也没有收益。

在最终保留实现上补测较大的 wave：32/64 个 M tile 的完整 8 卡 median
分别为 15.388/15.816 ms，加速比 1.413×/1.372×，全部通过原有正确性门。
测量期功耗中位数仍约 893/892 W，频率仍有下降，因此继续保留 wave=16。
实验日志、脚本和快照均留在 `/tmp/kimi_forward_perf`。

直接从远端 FC2 对称缓冲区读取并归约的实验也未保留到正式实现。地址映射通过
九组主机独立对照，但默认编译策略下，单链和四链版本均超过 300 秒后终止；
没有执行 NPU kernel。编译 pass 追踪停在 `hivm-plan-memory-regbase`。
同一单链输入改用 `plan_memory_strategy=largest-first` 后约 45 秒编译成功，
随后完整 8 卡的 normal、empty、all-drop 和三次更换 hidden states 的检查均通过。
标准 50 次测量的 median 为 23.872 ms（Torch 21.915 ms，0.918×）；同样采用
单链归约和 `largest-first` 的普通回传对照为 15.212 ms（Torch 21.720 ms，1.428×）。
直接拉取测量期采样频率均为 1650 MHz，功耗中位数约 821.5 W，仍明显更慢。
两组是分次测量；地址表构建、对称内存分配和通信方式同时变化，不能把全部差额
精确归给某一条远端读取指令。正式实现继续使用普通回传。

`largest-first` 不能直接作为通用配置：四链直接拉取和四链普通回传分别报告
UB 需求 1897472/1884160 bits，超过该编译配置可用的 1769472 bits。关闭自动
多缓冲、或换用配套 NPU-IR 编译器的单链输入均未在 120 秒编译预算内完成。
这些是新原型的编译结果，与已解决的 UDMA 设备库混用、早先的
`rtsGetHardwareSyncAddr` 启动异常属于不同问题。编译输入、命令、超时终止记录、
硬件测量与频率采样见测量记录中的 `direct_pull_reduce_experiments`。
